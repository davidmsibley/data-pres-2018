(function (global, factory) {
  ...
  class TableLayout extends litElement.LitElement {
    ...
    render() {
      ...
      let entries = Object.entries(this.info).filter((el) => {
        ...
        return !!el[value];
      }).map((el, index) => litElement.html`<dt class="label" title="${(keyLookup[el[key]])?keyLookup[el[key]].desc:el[key]}"><label for="${this.genId(index)}">${(keyLookup[el[key]])?keyLookup[el[key]].title:el[key]}</label></dt><dd class="detail" title="${(keyLookup[el[key]])?keyLookup[el[key]].desc:el[key]}"><span id="${this.genId(index)}">${el[value]}</span></dd>`);
  ...
  class QuatCoreLayout extends litElement.LitElement {
    ...
    render() {
      return litElement.html`<table-layout .info="${this.prepInfo()}"></table-layout>`;
    }
  ...
  class SiteDetails extends litElement.LitElement {
    ...
    render() {
      ...
      return litElement.html`${(!this.siteinfo)? '' : litElement.html`<div class="header"><span><a href="${window.router.link('entry')}" onclick="event.preventDefault()"><i class="material-icons clear-selection" title="Clear selection" @click="${this.fireClearSelection}">arrow_back</i></a></span><h1>${this.siteinfo.Site_Name}</h1><span><i class="material-icons zoom-to-site" title="Zoom to site" @click="${this.fireZoomToSite}">my_location</i></span></div>${this.renderData({
        ...
        })} ${this.siteinfo.datas.map((props) => litElement.html`<app-collapsible open><span slot="header" class="name">${props['Data_Type']}</span><div slot="content">${this.renderData(props, props['Data_Type'])}</div></app-collapsible>`)}`}`;
    }
  ...
  class FilterSummary extends litElement.LitElement {
    ...
    render() {
      return (!this.counts)?litElement.html``:litElement.html`<div><span>Showing ${this.counts.reduce((prev, count) => (count.current + prev), 0)} of ${this.counts.reduce((prev, count) => (count.total + prev), 0)} total sites</span><ul>${this.counts.map((el) => litElement.html`<li ?disabled="${!el.included}">${el.current} of ${el.total} <span class="name">${el.name}</span> sites ${(el.filteredBy.length > 0)?litElement.html`(filtered by ${el.filteredBy.join(', ')})`:''}</li>`)}</ul></div>`;
    }
  ...
  class MapFilter extends litElement.LitElement {
    ...
    renderFilterGroups() {
      let name=0, config=1;
      return this.filterGroups.map((group, index) => litElement.html`<slot name="${index}"></slot><app-collapsible class="group" ?open="${group.open}" @open="${this._handle(group)}"><i slot="header-before" class="material-icons collapse-icon"></i> <span slot="header">${group.title}</span> ${(!group.toggleable)?'':litElement.html`<toggle-switch name="${group.title}" slot="header-after" ?checked="${group.active}" @change="${this._handleGroup(group, 'include')}" style="--palette-active:${group.color}"></toggle-switch>`}<div slot="content">${group.sections.map((section, index) => litElement.html`${!(section.title)?'':litElement.html`<h2 class="section-title">${section.title}</h2>`} ${Object.entries(section.fields).map((entry, index) => litElement.html`<div class="field">${(entry[config].controls.length === 0)?'':entry[config].controls.map(control => litElement.html`<td class="label"><label for="${this.genId(index)}">${this.resolveKeyLookup(entry[name])}</label></td><td class="selector" @change="${this._handleControl(group, control, 'filter')}"><input type="hidden" id="${control.id}" name="${entry[name]}"> ${control.next}</td>`)}</div>`)}`)} ${(group.source && group.source.user)?litElement.html`<button-link href="${group.source.user}" class="raw-source-button"><span slot="content">View data source</span> <i slot="content-after" class="material-icons">open_in_new</i></button-link>`:''}</div></app-collapsible>`);
    }
